...
if(isset($_POST['addvideo']))
{
	...
	$type=1;
	$pdo=getPDOObject();
	$sql=$pdo->query("SELECT * FROM `video` where  name LIKE '$name'");
	$num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['photo']['name']);		
				...
				$q=$pdo->prepare("INSERT into `video` values(:id,:name, :photo,:des, :fld_order, :actstat)");
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `video` where id in ($str_rest_refs)");
		foreach($data as $video)
		{
			$img_path='../upload/'.$video['photo'];
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `video` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `video` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `video` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	$pdo=getPDOObject();
		$q=$pdo->prepare("UPDATE `video` SET 
...
function video_form($pid='0',$name='',$photo='',$des='',$fld_order='0',$actstat='',$formname='addvideo')
{ ?>
	<form action="video.php" method="post" enctype="multipart/form-data">
			   ...
			   <label>Name</label>
				...
				<textarea  name="des" cols="60" rows="10"><?php echo $des; ?></textarea><br />
					<script>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Edit video</h2>
                <div class="box-icon">
            ...
            <div class="box-content row" >
               ...
               <?php 
			$pid=$_GET['pid'];
			$productdata=sqlfetch("SELECT * FROM `video` where id='$pid' ");
			foreach($productdata as $product)
			{
				extract($product);
				video_form($pid,$name,$photo,$des,$fld_order,$actstat,$formname='editdone');
			} ?>
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Add video</h2>
                <div class="box-icon">
                    <a href="#" class="btn btn-setting btn-round btn-default"><i
                            class="glyphicon glyphicon-cog"></i></a>
                    <a href="#" class="btn btn-minimize btn-round btn-default"><i
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>Video</h2>
            ...
            <div class="box-content">
                <table class="table table-bordered table-striped responsive">
					<tbody>
						...
						<?php 
						$count=1;
						$data=sqlfetch("SELECT * FROM `video`  order by fld_order");
						...
						<tr>
							...
							<td>
								...
								<a class="ajax-link" href="video.php?&pid=<?php echo $menu['id']; ?>&edit=true">
								<button type="button" class="btn btn-xs btn-danger pull-right" name="editvideo">Edit</button>
								</a>
							</td>
						</tr>
						<?php } ?>
